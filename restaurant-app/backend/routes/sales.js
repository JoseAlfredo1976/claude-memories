...
// POST /api/sales - Create sale and update stock
router.post('/', (req, res) => {
  const db = getDb();
  const { sale_date, notes, items } = req.body;

  const getIngredients = db.prepare(`
    SELECT ri.product_id, ri.quantity, p.name as product_name, p.current_stock
  ...
  const insertSale = db.prepare(
    'INSERT INTO sales (sale_date, total_amount, notes) VALUES (?, ?, ?)'
  );
  const insertItem = db.prepare(`
    INSERT INTO sale_items (sale_id, recipe_id, recipe_name, quantity, unit_price, total_price)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
